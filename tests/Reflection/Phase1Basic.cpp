...
suite<"NGIN::Reflection::Phase1"> reflPhase1 = [] {
  ...
  "Type_Of_InferName_And_Fields"_test = [] {
    auto t = type_of<User>();
    expect(t.valid()) << "type_of<User> must be valid";
    expect(eq(t.qualified_name(), std::string_view{NGIN::Meta::TypeName<User>::qualifiedName}))
      << "qualified name should default from Meta::TypeName";
    expect(eq(t.field_count(), NGIN::UIntSize{2})) << "two fields registered";

    expect(eq(t.field_at(0).name(), std::string_view{"id"}));
    expect(eq(t.field_at(1).name(), std::string_view{"score"}));

    User u{};
    *static_cast<int*>(t.field_at(0).get_mut(&u)) = 42;
    expect(eq(u.id, 42)) << "mutable access via handle";
  };

  "Type_SetName_And_FieldAlias"_test = [] {
    auto t = type_of<Named>();
    expect(eq(t.qualified_name(), std::string_view{"My::Named"}));
    auto f = t.field("v");
    expect(f.has_value());
  };
};

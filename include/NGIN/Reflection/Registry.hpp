...
template<class M> struct MemberPtrTraits;
template<class C, class M> struct MemberPtrTraits<M C::*> { using Class = C; using Member = M; };
...
static std::expected<void, Error> FieldStore(void* obj, const class Any& value) {
  ...
  if (value.type_id() != tid) return std::unexpected(Error{ErrorCode::InvalidArgument, "type-id mismatch"});
  auto* c = static_cast<C*>(obj);
...
NGIN::UInt32 EnsureRegistered() {
  ...
  const auto tid = NGIN::Hashing::FNV1a64(sv.data(), sv.size());
  if (auto* p = reg.byTypeId.GetPtr(tid)) return *p;
...
public:
  ...
  [[nodiscard]] NGIN::UInt64 type_id() const;
  [[nodiscard]] NGIN::UIntSize size_bytes() const;
  [[nodiscard]] NGIN::UIntSize align_bytes() const;

  [[nodiscard]] NGIN::UIntSize field_count() const;
  [[nodiscard]] Field field_at(NGIN::UIntSize i) const;
  [[nodiscard]] ExpectedField field(std::string_view name) const;

  [[nodiscard]] NGIN::UIntSize method_count() const;
  [[nodiscard]] Method method_at(NGIN::UIntSize i) const;
